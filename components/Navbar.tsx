...
const Navbar: React.FC<NavbarProps> = ({ currentTab, setTab, manuals, isDark, toggleDark }) => {
  const tabs = [
    ...
    { id: 'quiz', label: 'Questionários', icon: '📝' },
    { id: 'assistant', label: 'IA Assistente', icon: '🤖' },
  ];
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex w-full md:w-auto justify-around items-center md:space-x-4">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setTab(tab.id)}
                className={`flex flex-col md:flex-row items-center space-y-1 md:space-y-0 md:space-x-2 px-3 py-1.5 rounded-xl transition-all
                  ${currentTab === tab.id 
